  ...
  useEffect(() => {
    if (data.status == "success" && data.isSuccess) {
      if (data.data.accounts.length <= 0) {
        toast.error("Para gönderebilmek için öncelikle bir banka hesabı açmalısınız.");
        setTimeout(() => {
      ...
      if (_accounts.length <= 0) {
        toast.error("Para gönderebilmek cari ve açık bir hesabınız bulunamadı!");
        setTimeout(() => {
          router.push("/management/accounts");
        }, 3000);
      }
              ...
              <Grid item container justifyContent="center" rowSpacing={5}>
                <Grid item xs={12} sm={6}>
                  ...
                  >
                    ...
                    <FormInput name={"description"} label={"Açıklama"} type={"text"} maxLength={255} />
